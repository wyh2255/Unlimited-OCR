...
def pdf_to_images(pdf_path: str) -> tuple[list[Image.Image], list[str], str]:
    """Render PDF pages to PIL Images. Returns (images, image_paths, tmp_dir)."""
    doc = fitz.open(pdf_path)
    tmp_dir = tempfile.mkdtemp(prefix="sglang_pp_")
    ...
    try:
        ...
        doc.close()
    return images, paths, tmp_dir


def parse_page_file(filepath: str) -> list[dict]:
    """Parse a per-page SGLang output file into a list of detections.

    Each detection: {full_match, label, bbox_999, content}
    """
    with open(filepath, encoding="utf-8") as f:
        text = f.read()

    # Pattern: <|det|>label [x1,y1,x2,y2]<|/det|>content
    ...
    for m in re.finditer(pattern, text, re.DOTALL):
        ...
        bbox_str = m.group(2)
        content = text[m.end():]
        # Find where the next tag starts (or end of text)
        next_tag = re.search(r"<\|det\|>", content)
        if next_tag:
        ...
        results.append({
            ...
            "label": label,
            "bbox_999": eval(bbox_str),  # [x1, y1, x2, y2] in 0-999 space
            "content": content,
            "match_end": m.end(),
            "det_start": m.start(),
        })
...
def main():
    ...
    page_images, page_paths, tmp_dir = pdf_to_images(args.pdf)
