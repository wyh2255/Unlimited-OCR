...
- **File**: `ocr_pdf.py` (146 lines).
- **Quirk**: images are NOT cropped or embedded by `ocr_pdf.py` itself; the model's own `save_results=True` emits `images/page_{N}_0.jpg` for pages that contain figures.
...
- **File**: `infer.py` (366 lines). Exposes two CLI functions: `main()` (CLI) and `run_inference(*, pdf, output_dir, concurrency, model_dir, gpu, image_mode, server_log) -> dict` (programmatic, used by Path 3).
...
- **Files**: `server.py` (482 lines) + `client.py` (498 lines).
- **Quirks**:
...
- **Hard dependency on CORS**: the browser blocks every API call unless `server.py` is started with `--cors-origin '*'` (or the specific frontend origin).
...
│   (direct)    │                │      10000)      │
       ...
       │                           ▼
       │                    ┌─────────────────┐
       │                    │  CLI client     │ (Path 4a,  client.py)
       │                    │  ocr-client     │ (Path 4b,  pip package)
       │                    │  Web frontend   │ (Path 5,   web/ → clients/web/)
       │                    └─────────────────┘
       │
...
- **Just want to OCR one PDF locally?** → Path 1 (`python ocr_pdf.py`).
- **Have a 50-page PDF and want it done in 2 minutes on one GPU?** → Path 2 (`python infer.py --concurrency 8`) + `python postprocess_sglang.py`.
- **Want to share a GPU across multiple laptops on the LAN?** → Path 3 (start `server.py` once, then `python client.py upload` from each laptop).
- **Want the same as Path 3 but with a UI?** → Path 4 (add `cd web && pnpm dev` to Path 3's setup, open `http://<server-laptop>:5173`).
...
`ocr-client/` is a packaging wrapper around the same logic as `client.py` (1 line intentional divergence: `prog=`). It's a dist of Path 4a, not a separate inference path. See [`ocr-client/README.md`](../ocr-client/README.md) (中文) for install.
