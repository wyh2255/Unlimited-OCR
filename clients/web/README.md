...
日期: 2026-06-26
文档类型: 用户使用手册
文档概述: Unlimited-OCR 局域网服务的浏览器前端 — Vue 3 + Vite,与 gateway/server.py / ocr-client 配合使用
---

# Unlimited-OCR Web 前端

针对 `ocr-client` 的浏览器版本。提供上传、状态轮询、结果在线预览(渲染 Markdown + 提取图片)三个主要流程,以及 token / server URL 等设置的本地保存。

## 1. 启动

### 1.1 启动后端(gateway/server.py)

先确认 `gateway/server.py` 已经启用了 CORS(默认 `*`)。
...
| Token (Bearer) | `gateway/server.py` 启动时打印的 `OCR_API_TOKEN` |
...
- 并发提示:勾选「使用服务器推荐值」就交给 `gateway/server.py` 按 GPU 空闲显存自动选;不勾则手动指定 1~16
...
确认 `gateway/server.py` 启动日志里有 `[server] CORS allow_origins=...`,且列表里包含你前端页面的 origin。也可以用 `--cors-origin` 显式声明,或者直接传 `*`(LAN 默认)。
...
**Q4. 「结果预览」里 PDF 没有图片。**
原 PDF 本来就没有可提取的位图,或者 `inference/postprocess.py` 没截到 bbox(模型对某些排版漏检)。这属于后端问题,不在前端范围。
...
| 操作 | ocr-client | Web |
|---|---|---|---|
| 上传 + 自动轮询 | `ocr-client upload doc.pdf --watch` | 「新建任务」tab → 上传 |
| 查询 | `ocr-client status <id>` | 「任务列表」自动轮询 |
| 下载 | `ocr-client download <id> --out ./out` | 「任务列表」→ 下载 ZIP,或「结果预览」在线看 |
| 删除 | `ocr-client delete <id>` | 「任务列表」→ 删除按钮 |
| 健康 | `ocr-client health` | 顶部常驻 |

`ocr-client` 的所有能力 Web 都有覆盖;`ocr-client` 的优势是无 GUI、可写脚本。
